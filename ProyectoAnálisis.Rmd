...
La percepción generalizada de que los veranos son cada vez más cálidos y los patrones climáticos más extremos plantea cuestiones fundamentales. ¿Se trata de meras impresiones subjetivas o existe realmente una tendencia climática cuantificable? Este análisis aborda científicamente esta cuestión mediante el examen de datos meteorológicos históricos de AEMET. Buscamos determinar si las variaciones térmicas responden a patrones consistentes, evaluar el impacto diferenciado por estaciones y localizaciones geográficas, y desarrollar modelos predictivos que permitan anticipar escenarios futuros en el contexto del cambio climático actual.
Los datos proceden de la Agencia Estatal de Meteorología (AEMET), con registros diarios de dos estaciones representativas del norte de España: San Sebastián (a 250 m de altitud) y Pamplona (a 459 m), abarcando el periodo 2000-2025. Esta selección es pertinente porque permite comparar la evolución climática en dos contextos geográficos contrastantes -costero versus interior- dentro de la misma región bioclimática, controlando así la variabilidad latitudinal mientras se examina el efecto de la continentalidad y altitud sobre las tendencias térmicas. El intervalo temporal de 25 años proporciona una base sólida para detectar tendencias significativas beyond la variabilidad interanual, esencial para estudios de cambio climático regional.
...
coef
```
Como podemos observar, la pendiente de la recta de regresión es positiva (0.05377831), lo que indica un aumento en la temperatura media diaria a lo largo de los años, sugiriendo una tendencia de calentamiento. 
Sin embargo es un muy pequeño, el aumento anual es de aproximadamente 0.054 °C por año, lo que sería un aumento de aproximadamente 1.35 °C en 25 años (de 2000 a 2025).
...
```
```{r}
summary(modelo_tmed)$coefficients["año", ]
summary(modelo_tmax)$coefficients["año", ]
summary(modelo_tmin)$coefficients["año", ]
```
El análisis confirma aumentos estadísticamente significativos (p < 0.05) en todas las temperaturas: la máxima crece 0.057 °C/año (p = 0.0007), la media 0.048 °C/año (p = 0.002) y la mínima 0.039 °C/año (p = 0.009).
...
Para evaluar si las temperaturas máximas y mínimas están aumentando al mismo ritmo, analizamos la evolución de la amplitud térmica (diferencia entre tmax y tmin) a lo largo del tiempo. Si la amplitud térmica aumenta, indica que las temperaturas máximas están creciendo más rápido que las mínimas.
```{r}
# Análisis de la amplitud térmica (diferencia tmax-tmin)
datos$amplitud_termica <- datos$tmax - datos$tmin
modelo_amplitud <- lm(amplitud_termica ~ fecha_num, data = datos)
summary(modelo_amplitud)$coefficients["fecha_num", ]

# Evolución de la amplitud térmica anual
amplitud_anual <- aggregate(amplitud_termica ~ año, data = datos, mean)
modelo_amplitud_anual <- lm(amplitud_termica ~ año, data = amplitud_anual)
summary(modelo_amplitud_anual)$coefficients["año", ]

```
El análisis de la amplitud térmica (tmax - tmin) revela un aumento estadísticamente significativo de 0.0182 °C por año (p = 2.59e-06) en los datos diarios y de 0.0187 °C por año (p = 0.0017) en las medias anuales. Esta ampliación progresiva del rango térmico confirma que las temperaturas máximas aumentan 0.018 °C/año más rápido que las mínimas, patrón característico del calentamiento global observado.

```{r}
# Visualización de la amplitud térmica anual
plot(amplitud_termica ~ año, data = amplitud_anual,
     ylab = "Amplitud térmica (°C)", 
     xlab = "Año",
     main = "Evolución de la amplitud térmica anual",
     pch = 16, col = "darkred")
abline(modelo_amplitud_anual, col = "red", lwd = 2)
```

El gráfico muestra una clara tendencia ascendente en la amplitud térmica anual, confirmando visualmente el aumento estadístico de 0.0187°C por año

# ¿El calentamiento afecta a todas las estaciones por igual?
Para responder a esta pregunta, primero vamos a representar las temperaturas medias diarias por estaciones del año mediante un boxplot. Luego, realizaremos un análisis de varianza (ANOVA) para comparar las medias de las temperaturas entre las diferentes estaciones del año y ver si hay diferencias significativas.
```{r}
boxplot(tmed ~ estacion, data = datos)
# Test ANOVA para comparar medias entre estaciones
modelo_anova <- aov(tmed ~ estacion, data = datos)
summary(modelo_anova)

#el p-value es extremadamente pequeño, por lo que la estación tiene un impacto estadisticamente significativo
#El calentamiento no afecta igual a todas las estaciones 
```
Para saber si el calentamiento afecta igual a todas las estaciones, no basta con comparar sólo las medias de temperatura, ya que por definición las estaciones registran valores diferentes. Lo apropiado es analizar si la tendencia de subida de temperatura es la misma en todas ellas.
Esto se puede hacer ajustando un modelo lineal de temperatura en función de la estación, el año y su interacción. Si la interacción es significativa, el calentamiento es distinto entre estaciones.
```{r}
modelo <- lm(tmed ~ estacion * año, data = datos)
summary(modelo)
```
Así, podemos concluir si el ritmo de calentamiento ha sido igual o distinto en cada estación.
En este caso, el p-valor asociado al término de interacción (estacion: año) es menor que 0.05, lo que indica que hay diferencias significativas en la tendencia de calentamiento entre las estaciones del año. Por lo tanto, podemos concluir que el calentamiento no afecta a todas las estaciones por igual.

## Variabilidad por estaciones de hora de temperatura máxima
...
coef
```
El coeficiente de la pendiente es de  0.0011 lo que indica que la hora de la temperatura máxima ha aumentado ligeramente a lo largo de los años en verano, aunque el cambio es muy pequeño (aproximadamente 0.0011 horas por año, o alrededor de 0.066 minutos por año).
### En otoño
...
coef
```
En otoño, sin embargo, el coeficiente es negativo (-0.0011), la temperatura máxima ha tendido a ocurrir un poco más temprano en el día a lo largo de los años, aunque nuevamente el cambio es muy pequeño.
### En invierno
```{r}
...
coef
```
Al igual que en otono, la pendiente es negativa, sin embargo su valore es algo mayor (-0.0015). Lo que indica que en invierno ha cambiado algo más la hora de temperatura máxima.
### En primavera
```{r}
...
coef
```
Por último, en primavera observamos que la pendiente vuelve a ser positiva (0.015) la mayor de todas con diferencia. Lo que indica que en esta estación encontramos el mayor cambio en la hora de la temperatura máxima a lo largo de los años.
Comparando las pendientes de las cuatro estaciones, podemos concluir que la hora de la temperatura máxima ha cambiado de manera diferente según la estación del año. En verano y primavera, la hora de la temperatura máxima ha tendido a ocurrir más tarde en el día, mientras que en otoño e invierno ha tendido a ocurrir más temprano. Sin embargo, es importante destacar que los cambios son bastante pequeños en todas las estaciones.
...
```
El p-valor obtenido es 0,1497 , que es mayor que el umbral común de 0.05. Por lo tanto, no rechazamos la hipótesis nula y concluimos que no hay evidencia suficiente para afirmar que la hora de la temperatura máxima ha cambiado significativamente entre el año 2000 y 2024.

# Regresión lineal múltiple para predecir la temperatura máxima diaria
Vamos a construir un modelo de regresión lineal múltiple para predecir la temperatura máxima diaria (tmax) utilizando varias variables predictoras disponibles en el conjunto de datos. Primero, examinamos las relaciones entre las variables mediante un gráfico de pares y luego ajustamos un modelo de regresión lineal múltiple.

```{r}
pairs(~tmax+tmin+año+mes+horatmax_decimal+horatmin_decimal+prec+velmedia+presMin+presMax+hrMedia, data=datos, pch = 16, cex = 0.5)
modelo_mult <- lm(tmax ~ tmin + año + mes + horatmax_decimal + horatmin_decimal + prec + velmedia + presMin + presMax + hrMedia, data = datos)
summary(modelo_mult)
vif(modelo_mult)
```
```{r}
...
# Calcular matriz de correlación y tomar valores absolutos
m <- abs(cor(datos[, c("tmax","tmin","tmed","año","mes",
                       "horatmax_decimal","horatmin_decimal",
                       "prec","velmedia","presMin","presMax","hrMedia")],
             use = "pairwise.complete.obs"))

colores <- colorRampPalette(c("#D8ECFF", "#8CCBFF", "#3C8DFF", "#1A4D8F"))(100)
...
corrplot(
  ...
  number.cex = 0.65,
  addCoef.col = "black",
  cl.lim = c(0, 1),  # Límites fijos entre 0 y 1
  type = "upper",
  diag = FALSE,
  is.corr = FALSE  # Importante: indicar que no es una matriz de correlación estándar
)
```

```{r}
predictors <- c("tmax","tmin","año","mes","horatmax_decimal","horatmin_decimal","prec","velmedia","presMin","presMax","hrMedia")
datos_complete <- datos[, predictors]
datos_complete <- datos_complete[complete.cases(datos_complete), ]
fit1 <- lm(tmax ~ tmin  + año + horatmax_decimal + horatmin_decimal + prec + velmedia + presMin + presMax + hrMedia, data = datos_complete)
...
modelo_mult <- lm(tmax ~ tmin + año + mes + horatmax_decimal + horatmin_decimal + prec + velmedia + presMin + presMax + hrMedia, data = datos2)
summary(modelo_mult)
vif(modelo_mult)
```
```{r}
...
# Calcular matriz de correlación y tomar valores absolutos
m <- abs(cor(datos2[, c("tmax","tmin","tmed","año","mes",
                       "horatmax_decimal","horatmin_decimal",
                       "prec","velmedia","presMin","presMax","hrMedia")],
             use = "pairwise.complete.obs"))
...
pred_inicio
```
El modelo predice que la temperatura máxima podría aumentar 0.41 °C para 2035 respecto al año inicial, con un intervalo de confianza entre 0.40 °C y 0.41 °C
...
pred_inicio2
```
El modelo predice que la temperatura máxima podría aumentar 3.91 °C para 2035 respecto al año inicial, con un intervalo de confianza entre 3.91 °C y 3.92 °C.

# Conclusiones
A modo de conclusion, el análisis confirma un calentamiento progresivo en ambas ciudades, más pronunciado en Pamplona (interior) que en San Sebastián (costa). Las temperaturas máximas aumentan más que las mínimas, y el calentamiento varía estacionalmente, siendo primavera y verano las más afectadas. Se observan cambios en los patrones horarios: la temperatura máxima ocurre más tarde en primavera/verano y más temprano en otoño/invierno. Las proyecciones indican una continuación de esta tendencia, con aumentos mayores en Pamplona. Estos resultados subrayan la necesidad de políticas de adaptación al cambio climático basadas en evidencias locales.


<script>
document.addEventListener("DOMContentLoaded", function() {
    var toc = document.getElementById("TOC");
    var target = document.getElementById("toc-lateral");
    if (toc && target) {
