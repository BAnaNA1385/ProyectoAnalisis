...
datos <- datos[!is.na(datos$tmin) & 
               !is.na(datos$tmed) & 
               !is.na(datos$tmax) & 
               !is.na(datos$horatmax_decimal) & 
               !is.na(datos$horatmin_decimal) &
               !is.na(datos$fecha_num), ]
```
###1###
...
```{r}
fit1  <- lm(tmax_decimal ~ ., data = datos2_clean)
fit2 <- lm(tmax_decimal ~ 1, data = datos2_clean)
modelAIC <- step(fit2, scope = list(lower=fit2, upper=fit1), direction="forward")
formula <- formula(modelAIC)
