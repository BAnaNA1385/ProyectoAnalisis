...
datos$horatmax <- as.character(datos$horatmax)
datos$horatmax[datos$horatmax == "varias"] <- NA         # If "varias" means "several" times or missing
datos$horatmax[datos$horatmax == ""] <- NA               # Empty strings as NA
datos$horatmax <- as.numeric(datos$horatmax)
datos$horatmax_valid <- !is.na(datos$horatmax)
datos$horatmin <- as.character(datos$horatmin)
datos$horatmin[datos$horatmin == "varias"] <- NA
datos$horatmin[datos$horatmin == ""] <- NA
datos$horatmin <- as.numeric(datos$horatmin)
...
modelo_final2<- lm(tmax ~ tmin + año + mes + horatmax + horatmin + prec + velmedia + presmin + presmax + hrMedia, data = datos2)
