...
# Convert temperature columns FIRST (before filtering)
...
# Convert hora columns (handle 'varias' → NA)
datos$horatmax <- as.numeric(as.character(datos$horatmax))
datos$horatmin <- as.numeric(as.character(datos$horatmin))

# Add fecha_num BEFORE filtering
datos$fecha_num <- lubridate::decimal_date(datos$fecha)

# NOW filter — keep rows with valid temperatures (allow NA in hora columns for now)
datos <- datos[!is.na(datos$tmin) & !is.na(datos$tmed) & !is.na(datos$tmax), ]

# Mark valid rows for hora columns (optional — use in seasonal analyses)
datos$horatmax_valid <- !is.na(datos$horatmax)
datos$horatmin_valid <- !is.na(datos$horatmin)
```
###1###
...
En verano
```{r}
datos_verano <- datos[datos$estacion == 'verano' & datos$horatmax_valid, ]
modelo  <- lm(horatmax ~ fecha_num, data = datos_verano)
coef = coefficients(modelo)
recta1 = coef[1] + seq(2000,2025,1)* coef[2]
x <- seq(2000,2025,1)
...
# Crear un subset con solo las variables de interés (no funciona bien sino)
vars_interes <- c("tmax", "tmin", "tmed", "año", "mes", 
                  "prec_num", "velmedia_num", "presMin", "presMax", "hrMedia")

# Eliminar filas con NA en cualquier de estas variables
datos2_clean <- na.omit(datos2[, vars_interes])

# Ahora generar el gráfico de pares
pairs(~ tmax + tmin + tmed + año + mes + 
        prec_num + velmedia_num + presMin + presMax + hrMedia, 
      data = datos2_clean, pch = 16, cex = 0.5)

modelo_mult2 <- lm(tmax ~ tmin + tmed + año + mes + prec_num + velmedia_num + presMin + presMax + hrMedia, data = datos2)

summary(modelo_mult2)
vif(modelo_mult2)
pairs(~tmax+tmin+tmed+año+mes+prec+velmedia+presmin+presmax+hrMedia, data=datos2, pch = 16, cex = 0.5)
modelo_mult2 <- lm(tmax ~ tmin + tmed + año + mes + horatmax + horatmin + prec + velmedia + presMin + presMax + hrMedia, data = datos2)
summary(modelo_mult2)
vif(modelo_mult2)
```
#Le he quitado, hrtmax, y hrtmin, porque no estaban bien hechas las conversiones 
```{r}
fit1  <- lm(tmax ~ ., data = datos2_clean)
fit2 <- lm(tmax ~ 1, data = datos2_clean)
modelAIC <- step(fit2, scope = list(lower=fit2, upper=fit1), direction="forward")
formula <- formula(modelAIC)
modelFinal <- lm(formula, data = datos2_clean)
summary(modelFinal)
vif(modelFinal)
...
med_horatmax <- median(datos$horatmax, na.rm = TRUE)
med_horatmin <- median(datos$horatmin, na.rm = TRUE)
...
modelo_final <- lm(tmax ~ tmin + año + mes + horatmax + horatmin + prec + velmedia + presmin + presmax + hrMedia, data = datos)
...
med_horatmax2 <- median(datos2$horatmax, na.rm = TRUE)
med_horatmin2 <- median(datos2$horatmin, na.rm = TRUE)
...
modelo_final2<- lm(tmax ~ tmin + año + mes + horatmax + horatmin + prec + velmedia + presmin + presmax + hrMedia, data = datos)
